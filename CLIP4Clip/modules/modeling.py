...
from modules.module_adapter import side_tuning_factory_for_transformer
...
class CLIP4Clip(CLIP4ClipPreTrainedModel):
    def __init__(self, cross_config, clip_state_dict, task_config):
        ...
        self.adapter_name = 'our_tuning'
        self.downsample_D_factor = 1
        assert self.adapter_name in ['our_tuning']
        self.image_adapter = side_tuning_factory_for_transformer[self.adapter_name](num_layer=13,
                                                                                    hidden_size=[768] * 13,
                                                                                    embed_size=embed_dim,
                                                                                    downsample_D_factor=self.downsample_D_factor)
        self.text_adapter = side_tuning_factory_for_transformer[self.adapter_name](num_layer=13,
                                                                                   hidden_size=[512] * 13,
                                                                                   embed_size=embed_dim,
                                                                                   downsample_D_factor=self.downsample_D_factor)
    ...
    def get_sequence_output(self, input_ids, token_type_ids, attention_mask, shaped=False):
        ...
        sequence_hidden = self.text_adapter(hidden_list, hidden.unsqueeze(1), 
                                            mask=mask, position=input_ids.argmax(dim=-1))
    ...
    def get_visual_output(self, video, video_mask, shaped=False, video_frame=-1):
        ...
        visual_hidden = self.image_adapter(hidden_list, hidden.unsqueeze(1))
