...
class ResnetFeatureExtractor(nn.Module):
    ...
    def forward(self, input):
        ...
        layer1_out = self.base[:5](input)
        layer2_out = self.base[5](layer1_out)
        layer3_out = self.base[6](layer2_out)
        layer4_out = self.top(layer3_out)

        return self._flat_featuremap(layer4_out), [layer1_out, layer2_out, layer3_out, layer4_out]
...
if __name__ == '__main__':
    ...
    backbone_cnn = ResnetFeatureExtractor('detector', '/home/dhw/DHW_workspace/project/vse_infty-master/data/weights/original_updown_backbone.pth', fixed_blocks=2)
    tensorA = torch.FloatTensor(128, 3, 448, 448)
    out, all = backbone_cnn(tensorA)
    print('finished')
