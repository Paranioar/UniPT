        ...
        if opt.precomp_enc_type == 'basic':
            if self.opt.optim == 'adam':
                self.optimizer = torch.optim.AdamW([
                    {'params': self.txt_enc.rest_need_train_params, 'lr': opt.learning_rate},
                    {'params': self.txt_enc.bert_need_train_params, 'lr': opt.learning_rate * opt.bert_lr_factor},
            ...
            if self.opt.optim == 'adam':
                self.optimizer = torch.optim.AdamW([
                    {'params': self.txt_enc.rest_need_train_params, 'lr': opt.learning_rate},
                    {'params': self.txt_enc.bert_need_train_params, 'lr': opt.learning_rate * opt.bert_lr_factor},
                    {'params': self.img_enc.rest_need_train_params, 'lr': opt.learning_rate},
                    {'params': self.img_enc.resnet_need_train_params, 'lr': opt.learning_rate * opt.backbone_lr_factor}
                ], lr=opt.learning_rate, weight_decay=decay_factor)
            else:
                raise ValueError('Invalid optim option {}'.format(self.opt.optim))
