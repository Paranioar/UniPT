...
def get_tuple(splits: str, bs:int, shuffle=False, drop_last=False, distributed=False, aspect_ratio_group_factor = -1, exhaustive = False) -> DataTuple:
...
class GQA(TrainerBase):
    ...
    def create_side_network_initial_weights(self):
        ...
        if "fisher" in self.args.load_side_pretrained_weights or "bert" in self.args.load_side_pretrained_weights:
            ...
            model = GQAModel(self.train_tuple.dataset.num_answers) # ad-hoc writing
            ...
            # prepare the dataset
            train_dataset = GQATorchDataset(GQADataset(self.args.train)) # ad-hoc writing
            ...
            if "fisher" in self.args.load_side_pretrained_weights:
                # ad-hoc writing
                importance_measure = compute_fisher(model, "gqa", train_dataset, data_collator, num_samples=self.args.samples_for_fisher)
            else:
