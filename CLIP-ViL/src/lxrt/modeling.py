...
class LXRTEncoder(nn.Module):
    def __init__(self, config):
        ...
        if config.side_config is not None:
            self.side_first_downsample_l, self.side_final_upsample_l, self.side_downsamples_l, \
                self.side_projection_l, self.side_gate_layer_l \
                    = self.construct_side_network(config, config.side_config, self.num_l_layers, BertLayer, 'l')           
    ...
    def construct_side_network(self, config, side_config, num_layers, LAYER_TYPE, gate_name):

        hidden_size = config.hidden_size // side_config.reduction_factor

        side_first_downsample = nn.Sequential(nn.Linear(config.hidden_size, hidden_size), 
                                              BertLayerNorm(hidden_size))

        side_downsamples = nn.ModuleList([nn.Sequential(nn.Linear(config.hidden_size, hidden_size),
                                                        BertLayerNorm(hidden_size)) for i in range(num_layers)])

        side_final_upsample = nn.Sequential(nn.Linear(hidden_size, config.hidden_size))

        side_projection = nn.Sequential(nn.Linear(hidden_size, config.hidden_size),
                                        nn.ReLU(),
                                        nn.Linear(config.hidden_size, hidden_size),
                                        BertLayerNorm(hidden_size))
                                
        side_gate_layer = nn.Linear(hidden_size, 1)

        return side_first_downsample, side_final_upsample, side_downsamples, side_projection, side_gate_layer
    ...
    def cross_attention(self, query, context, mask=None, **kwargs):
    ...
    def forward(self, lang_feats, lang_attention_mask,
        ...
        if self.train_side_transformer:
            all_downsample_features = []

            mask = (joint_mask.squeeze(1).squeeze(1) == 0).to(dtype=visn_feats.dtype).to(visn_feats.device)
            all_downsample_features.append(self.side_first_downsample_l(joint_feats))
        ...
        for idx, layer_module in enumerate(self.layer):
            ...
            if self.train_side_transformer:
                all_downsample_features.append(self.side_downsamples_l[idx](joint_feats))

        if self.train_side_transformer:

            assert len(all_downsample_features) == len(self.layer) + 1 

            confidence_set = []
            merged_features_set = []

            anchor_features = all_downsample_features[-1]
            anchor = self.aggregate_feature(anchor_features, mask=mask)
            
            for index in range(len(self.layer)):

                target_features = all_downsample_features[index]
                target = self.aggregate_feature(target_features, mask=mask)
                merged_features, _ = self.cross_attention(anchor_features, target_features, mask=mask)

                merged_features_set.append(merged_features)
                confidence_set.append(self.side_gate_layer_l(anchor * target))
            ...
            output_features = anchor_features + self.side_projection_l(merged_features)
        
            joint_feats = self.side_final_upsample_l(output_features)
